...
сравнением со скриншотами-эталонами (Linux-рендер). Тест гоняется в CI
поверх статики, закоммиченной в `build/`, — харнесс не пересобирает код
студента сам, поэтому после правок нужно запускать `make build` и
коммитить результат.
